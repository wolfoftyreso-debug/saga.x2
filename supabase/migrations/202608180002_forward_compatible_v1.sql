-- Safe to run after an earlier copy of 202608180001_initial_schema.sql.
-- Fresh installations also run this harmlessly after the initial schema.
alter table public.events
  add column if not exists latest_short_term_impact text,
  add column if not exists latest_long_term_impact text;

alter table public.brief_items
  add column if not exists direct_alert boolean not null default false;

drop function if exists public.publish_daily_brief(uuid, uuid, date, text, text, jsonb, jsonb);
...
begin
  ...
  loop
    ...
    -- The TypeScript gate classifies source types from the source URL before
    -- calling this service-role-only function.
